...
public int calcularDiasEstadia(Date fechaIngreso, Date fechaSalida) {
    ...
    Calendar cal = dcfecha_reserva.getCalendar();
    if (cal != null) {
    ...
    fhabitacion funcHabitacion = new fhabitacion();
    vhabitacion habitacion = funcHabitacion.buscarPorId(Integer.parseInt(txtidhabitacion.getText())); 

    if (habitacion != null) {
        dts.setCosto_alojamiento(habitacion.getPrecio_diario() * calcularDiasEstadia(dts.getFecha_ingresa(), dts.getFecha_salida()));
        txtcosto_alojamiento.setText(String.valueOf(dts.getCosto_alojamiento()));
    }
    ...
    JOptionPane.showMessageDialog(rootPane, "Error en el campo ID de Habitación: " + e.getMessage());
    txtidhabitacion.requestFocus();
    return;
}
